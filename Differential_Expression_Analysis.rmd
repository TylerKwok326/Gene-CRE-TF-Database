...
```{r}
WTC11_results_ENSG <- convert_entrez_to_ensembl(WTC11_results, 'entrez')
```
```{r}
WTC11_genes_list <- cbind(WTC11_results_HGNC, WTC11_results_ENSG)
WTC11_genes_list <- WTC11_genes_list %>%
  dplyr::select('hgnc','ensembl_gene_id','entrez')
```

```{r}
WTC11_gene_positions <- get_gene_coo
```
...
# BED files
```{r}
WTC11_cres <- read.delim("/Users/nathan/Desktop/Bioinformatics/Projects/AD_Database/Gene_Mapping/results/WTC11.closest.bed", header = FALSE, sep = "\t")
H1_cres <- read.delim("/Users/nathan/Desktop/Bioinformatics/Projects/AD_Database/Gene_Mapping/results/H1.closest.bed", header = FALSE, sep = "\t")
```

```{r}
# All CREs
all_cres <- read_tsv("/Users/nathan/Desktop/Bioinformatics/Projects/AD_Database/Gene_Mapping/refs/TFBSlog2FCsum_mergedCRE_deAD_sigTF_top0.05dis_neat.txt")
```

```{r}
names(all_cres)[3:ncol(all_cres)] <- names(all_cres)[3:ncol(all_cres)] %>%
  str_replace_all("_.*?_", "_")
```

```{r}
unique_cres <- all_cres[, 1:3]

```
```{r}
cres_tf <- all_cres[, 4:2106]
```

```{r}
write.csv(unique_cres, 'unique_cres.csv')
```
```{r}
write.csv(cres_tf, 'cres_tf.csv')
```
